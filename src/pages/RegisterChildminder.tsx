  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    supabase.functions.invoke("send-contact-email", {
      body: {
        ...
        email: form.email,
        message: [
          `NEUE ANMELDUNG BETREUUNGSPERSON`,
          `Telefon: ${form.phone}`,
          `Adresse: ${form.address}`,
          `PLZ: ${form.postcode}`,
          `Arbeitserlaubnis bestätigt: ${form.rightToWork ? "Ja" : "Nein"}`,
          `Erweitertes Führungszeugnis (§ 30a BZRG): ${form.hasDbs ? `Ja — Nr. ${form.dbsNumber}` : "Nein / noch nicht vorhanden"}`,
          `Erfahrung: ${form.experienceSummary || "Keine Angabe"}`,
          `Verfügbarkeit: ${form.availabilityNotes || "Keine Angabe"}`,
        ].join("\n"),
      },
    }).catch(() => {});
