...
serve(async (req) => {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (session.metadata?.kind === "booking" && session.metadata.target_id) {
      ...
      if (session.metadata?.kind === "training_course" && session.metadata.target_id && session.metadata.user_id) {
