...
const PRODUCT_PLANS: Record<string, string> = {
  "prod_U768nOl4huuKpY": "basic_monthly",
  "prod_U768mppY1TLmcn": "basic_annual",
  "prod_U768PwKx6DIq4g": "training_monthly",
  "prod_U768CRtE4wJcUC": "training_annual",
};
...
serve(async (req) => {
  ...
  try {
    ...
    // Find basic plan first, then training
    let basicSub = null;
    ...
    const activeSub = basicSub || trainingSub || { sub: subscriptions.data[0], planName: "unknown" };
    const subscriptionEnd = new Date(activeSub.sub.current_period_end * 1000).toISOString();

    logStep("Active subscription", { plan: activeSub.planName, end: subscriptionEnd });

    return new Response(JSON.stringify({
      subscribed: true,
      plan: activeSub.planName,
      subscription_end: subscriptionEnd,
      has_training: !!trainingSub,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
